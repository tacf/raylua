...
source: https://github.com/raysan5/raylib/blob/master/examples/gui/gui_portable_window.c
...
local mouse_pos = rl.rlVertex2f(0, 0)
local window_pos = rl.GetWindowPosition()
local pan_offset = rl.rlVertex2f(mouse_pos)
...
while not exit_window and not rl.WindowShouldClose() do
  ...
  if rl.IsMouseButtonPressed(rl.MOUSE_BUTTON_LEFT) then
    if rl.CheckCollisionPointRec(mouse_pos, rl.new("Rectangle", 0, 0, width, 20)) then
      drag_window = true
      pan_offset = rl.rlVertex2f(mouse_pos)
    end
  end
