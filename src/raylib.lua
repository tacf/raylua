...
local rl = package.loaded["raylib"]
if not rl then
  rl = {}
  package.loaded["raylib"] = rl
  package.preload["raylib"] = function()
    return rl
  end
end
...
do
  ...
  while entries[i].name ~= NULL do
    ...
    local ok, casted = pcall(ffi.cast, proto, entries[i].ptr)
    if not ok then
      error(string.format("bad ffi entry %s :: %s", name, proto))
    end
    rl[name] = casted
    i = i + 1
  end
